...
export default class Eintrag {
     ...
     * @prop {Number} _timestamp - der Unix-Zeitstempel der Instanziierung
    ...
    eintrag_entfernen_event_hinzufuegen(listenpunkt) {
        listenpunkt.querySelector(".entfernen-button").addEventListener("click", (event) => {
            event.preventDefault()
            liqui_planner.eintrag_entfernen(this._timestamp)
          })
    }
     ...
     * @returns {Element} - das Eintrags-Element mit all seinen Kindelementen und dem Click-Event
     */
    _html_generieren() {
        ...
        betrag.textContent = `${(this._betrag / 100).toFixed(2)}€`
